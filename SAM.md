...
```bash
switchbot-webhook$ sam build --use-container
```
...
```bash
switchbot-webhook$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
switchbot-webhook$ sam local start-api
switchbot-webhook$ curl http://localhost:3000/
```
...
```bash
switchbot-webhook$ sam logs -n HelloWorldFunction --stack-name switchbot-webhook --tail
```
...
```bash
switchbot-webhook$ pip install -r tests/requirements.txt --user
# unit test
switchbot-webhook$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
switchbot-webhook$ AWS_SAM_STACK_NAME="switchbot-webhook" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "switchbot-webhook"
```
